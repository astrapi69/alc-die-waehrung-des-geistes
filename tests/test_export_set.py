...
* the manifest set id resolves the same set as the path basename slug
  (both are ``waehrung-des-geistes`` in this repo),
* ``--split-size N`` splits a large set's export into multiple
  self-contained part files (each with its own ``review_instructions``
  copy plus ``part``/``of``/``lesson_count``/``total_lesson_count``),
  and ``chunk_lessons()`` is covered directly as a pure unit.
"""
...
KNOWN_SLUG = "waehrung-des-geistes"
KNOWN_SET_DIR = REPO_ROOT / "sets" / "de" / "waehrung-des-geistes"
...
def test_manifest_id_resolves_like_path_basename(tmp_path: Path) -> None:
    # In this repo the manifest set id and the path basename coincide
    # (waehrung-des-geistes), so the exact-id branch resolves first.
    out_path = tmp_path / "by-id.yaml"
    exit_code = export_set.main(["waehrung-des-geistes", "--out", str(out_path)])
    assert exit_code == 0
    export_payload = yaml.safe_load(out_path.read_text(encoding="utf-8"))
    assert export_payload["set"] == "waehrung-des-geistes"
    assert export_payload["lessons"] == load_source_lessons()
...
def test_split_size_writes_one_file_per_chunk_with_correct_lesson_counts(
    ...
    source_lessons = load_source_lessons()
    assert len(source_lessons) == 11  # KNOWN_SLUG fixture assumption

    created_files = run_split_export(tmp_path, 6)
    ...
    assert part_1["lesson_count"] == 6
    assert part_2["lesson_count"] == 5
    assert part_1["total_lesson_count"] == 11
    assert part_2["total_lesson_count"] == 11
...
def test_split_size_parts_concatenate_to_the_full_lesson_order(tmp_path: Path) -> None:
    source_lessons = load_source_lessons()
    created_files = run_split_export(tmp_path, 4)
